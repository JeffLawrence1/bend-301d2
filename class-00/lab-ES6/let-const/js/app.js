...
const allProducts = [];  //left alone because const arrays can still have their contents altered
const container = document.getElementById('image_container');
const viewed = [];
const labels = [];
const pics = [leftImage, centerImage, rightImage];
const list = document.getElementById('productlist');
let totalClicks = 0; //changed const to let
const views = [];
const votes = [];
...
function displayPics(){
  ...
  // PUT YOUR RESPONSE IN THIS COMMENT --- the console log could no longer access the variable rando because it was declared inside the displaypics function using let which means that it is only available inside the displaypics function
  console.log(viewed);

  for (let i = 0; i < 3; i++){  //turned const back to let
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){  //const to let
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {  //changed const to let
...
if(localStorage.busmall){
  ...
  for(let i = 0; i < names.length; i++) {   //turned const back to let
    new Product(names[i]);
  }
}
